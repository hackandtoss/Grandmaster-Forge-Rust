...
mod tests {
    ...
    fn repertoire_tree_insert_and_lookup() {
        ...
        assert_eq!(
            ...
            Some(("White".to_string(), start.to_string()))
        );
        assert_eq!(store.get_node_side_and_fen(9999).unwrap(), None);
    }
    ...
    fn lichess_sync_migrates_to_game_sync() {
        let mut store = SqliteStore::new_in_memory().unwrap();
        store.bootstrap().unwrap();
        // Simulate a legacy DB that still carries the old single-source ledger.
        store.conn.execute(
